...
func _process(delta):
	...
	if is_moving == true:
		...
		if now - last_movement_ms > movement_ms:
			...
			if self.is_in_group( "line_"+str(line_number) ) == false:
				print("ADDED TO " + "line_"+str(line_number))
				self.add_to_group("line_"+str(line_number))
